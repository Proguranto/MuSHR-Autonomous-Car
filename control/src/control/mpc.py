...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        # BEGIN QUESTION 4.1
        "*** REPLACE THIS LINE ***"
        se = np.linspace(self.min_delta, self.max_delta, num=self.K)
        controls[:,:,1] = se[:,np.newaxis]
        # END QUESTION 4.1
        return controls
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for t in range(self.T):
            # self, states, controls, dt, delta_threshold
            change = self.motion_model.compute_changes(rollouts[:, t, :], controls[:, t, :], dt)
            next = rollouts[:, t, :] + change
            rollouts[:, t + 1, :] = next
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        # distance weighted by self.error_q
        final_rollout = rollouts[:, -1, :2]
        reference_xy = reference_xyt[:2]
        distance = np.linalg.norm(final_rollout - reference_xy, axis=1)
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # one cost for each rollout(K rollouts)
        costs = np.empty((self.K))

        # collision cost weighted by self.collision_w
        collisions = self.check_collisions_in_map(rollouts.reshape(-1, 3))
        collisions = collisions.reshape(self.K, self.T + 1)
        count_collisions = np.sum(collisions, axis=1)

        costs = count_collisions * self.collision_w
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        # return the first action of the control sequence that minimizes cost.
        # velocity and steering angle
        minimized_index = np.argmin(costs)
        # print(minimized_index)
        minimized_controls = self.sampled_controls[minimized_index]

        control = np.empty(2)
        # set velocity
        control[0] = self.sampled_controls[0,0,0]

        # set the steering angle
        # minimized_rollout is a sequences that all have same steering angle? (rollout is (x, y, theta))
        control[1] = minimized_controls[0,1]
        # print(control)
