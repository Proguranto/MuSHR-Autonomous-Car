...
def compute_position_in_frame(p, frame):
    ...
    first[0, 0] = np.cos(theta_F)
    first[0, 1] = np.sin(theta_F)
    first[1, 0] = -np.sin(theta_F)
    first[1, 1] = np.cos(theta_F)

    second = np.array([x - x_F, y - y_F])
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            "*** REPLACE THIS LINE ***"
            L, _ = path_xytv.shape
            path_xytv_copy = path_xytv[self.last_path_idx:]

            wp_idx = np.linalg.norm(path_xytv_copy[:, :2] - pose[:2], axis=1).argmin()
            if wp_idx == L - 1:
                self.last_path_idx = L - 1
                return L - 1
            
            sliced_path = path_xytv_copy[wp_idx:, :2]
            cum_sum = np.cumsum(np.linalg.norm(np.diff(sliced_path, axis=0), axis=1), axis=0) - distance_lookahead
            if cum_sum[-1] < 0:
                self.last_path_idx = L - 1
                return L - 1
            
            ref_idx = np.abs(cum_sum).argmin()

            self.last_path_idx = wp_idx

            return ref_idx + wp_idx + 1
            # END QUESTION 1.1
