...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        # grab x y from message
        x = msg.position.x
        y=msg.position.y

        # turn quartenion into angles
        angle = utils.quaternion_to_angle(msg.orientation)

        # remember that at the beginning, each particle weighted similarly
        weights[:] = np.full_like(weights, 1.0/n_particles)

        # initialize normally distributed particles around the message position
        particles[:, 0] = x + self.x_std*np.random.normal(size=n_particles)
        particles[:, 1] = y + self.y_std*np.random.normal(size=n_particles)
        particles[:, 2] = angle + self.theta_std*np.random.normal(size=n_particles)
