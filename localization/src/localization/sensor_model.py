...
class SingleBeamSensorModel:
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            **kwargs (object): any number of optional keyword arguments:
                ...
                z_short (float): Weight for short reading (unexpected objects)
                z_max (float): Weight for max reading  (max sensor distance)
    ...
    def precompute_sensor_model(self, max_r):
        ...
        # BEGIN QUESTION 2.1
        "*** REPLACE THIS LINE ***"
        case1 = ((obs_r >= 0 )& (obs_r <= max_r))
        case2 = (obs_r < sim_r)
        case3 = (obs_r == max_r)
        case4 = ((obs_r >= 0)&(obs_r < max_r))
        if self.hit_std>0:
            prob_table[case1] = self.z_hit * np.exp(-0.5 * (diff[case1]/ self.hit_std) ** 2) / np.sqrt(2 * np.pi * (self.hit_std ** 2))
        prob_table[case2] += self.z_short * 2 * diff[case2] / np.square(sim_r[case2])
        prob_table[case3] += self.z_max
        if max_r > 0:
            prob_table[case4] += self.z_rand / max_r

        # for i in range (table_width):
        #     for j in range(table_width):
        #         curr_diff = diff[i][j]

        #         if obs_r[i][j] >= 0 and obs_r[i][j] <= max_r and self.hit_std>0:
        #             # Calculate p_hit
        #             prob_table[i][j] += self.z_hit * np.exp(-0.5 * (curr_diff / self.hit_std) ** 2) / np.sqrt(2 * np.pi * (self.hit_std ** 2)) 

        #         # # Calculate p_short
        #         if obs_r[i][j] < sim_r[i][j]:
        #             prob_table[i][j] += self.z_short * 2 * diff[i][j] / (sim_r[i][j] ** 2)

        #         # # Calculate p_max
        #         if obs_r[i][j] == max_r:
        #             prob_table[i][j] += self.z_max

        #         # Calculate p_rand    
        #         if obs_r[i][j] >= 0 and obs_r[i][j] < max_r:
        #             prob_table[i][j] += self.z_rand / max_r
        #         if obs_r[i][j] > max_r or obs_r[i][j]<0:
        #             prob_table[i][j] = 0
